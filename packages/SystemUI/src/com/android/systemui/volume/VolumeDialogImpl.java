...
public class VolumeDialogImpl implements VolumeDialog, Dumpable,
    ...
    private void addAppRow(AppVolume av) {
        ...
        row.dndIcon.setVisibility(GONE);
        mDialogRowsView.addView(row.view);
        mRows.add(row);
        mAppRows.add(row);
    ...
    private void initRow(final VolumeRow row, final int stream, int iconRes, int iconMuteRes,
        ...
        if (row.isAppVolume) {
            ...
            appIconR.setFromDegrees(-270);
            appIconR.setToDegrees(-270);
            seekbarProgressDrawable.setDrawableByLayerId(
                    R.id.volume_seekbar_progress_icon, appIconR);
        } else {
    ...
    private void initSettingsH(int lockTaskModeState) {
        ...
        if (mExpandRows != null) {
            mExpandRows.setOnClickListener(v -> {
                ...
                if (mExpansionState != ExpansionState.EXPANDED && !mVolumePanelOnLeft) {
                    rotateIcon();
                } else if (mExpansionState != ExpansionState.EXPANDED && mVolumePanelOnLeft) {
                    rotateIconReverse();
                } else if (mExpansionState == ExpansionState.EXPANDED && mVolumePanelOnLeft) {
                    rotateIcon();
                } else {
    ...
    public void initAppVolumes() {
        ...
        int visibility = showActiveMedia ? VISIBLE : GONE;
        if (mAppVolumeView != null) {
            mAppVolumeView.setVisibility(visibility);
        }
        if (mAppVolumeIcon != null && showActiveMedia) {
    ...
    private void updateRowsH(final VolumeRow activeRow, boolean animate) {
        ...
        for (final VolumeRow row : mRows) {
            ...
            final boolean shouldBeInvisible =
                    (isExpandableRow && appsExpanded) || (isAppRow && expanded) || collapsed;

            if (!isExpandableRow && !isAppRow) {
            ...
            if (row.view.isShown() || isExpandableRow || isAppRow) {
        ...
        if (outmostVisibleRowIndex > -1 && outmostVisibleRowIndex < Short.MAX_VALUE) {
            ...
            for (final VolumeRow row : mRows) {
                final boolean isStreamRow = isExpandableRowH(row) && expanded;
                final boolean isAppRow = row.isAppVolume && appsExpanded;
                ...
                final int translationMultiplier = (int) getTranslationForPanelLocation();
                ...
                if (isAppRow) {
                    ...
                    final int appRowsOffset = translationMultiplier * getExpandableRowsExtraSize();
                    fromTranslationX += appRowsOffset;
                    toTranslationX += appRowsOffset;
                }
    ...
    private void updateAppVolumeRow(VolumeRow row, boolean setProgress) {
        ...
        if (setProgress) {
            ...
            row.slider.setProgress(progress, true);
        }
